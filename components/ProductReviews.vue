...
async function loadReviews() {
  ...
  const response = await deskree.reviews.get(props.productId);
...
async function handleReviewSubmit(form) {
  ...
  const response = await deskree.reviews.submit({
    ...form,
...
<template>
  ...
  <div class="my-10" v-if="status === 'loaded'">
    ...
    <div class="flex items-center">
      ...
      <div class="pl-5">
        <div v-for="number in 5" class="text-xs flex items-center">
          {{ number }} Stars
          ...
          <input
            ...
            max="100"
            :value="
              groupedReviews[number]
                ? (groupedReviews[number].length / reviews.length) * 100
                : 0
            "
            disabled
          />
          ({{ groupedReviews[number]?.length || 0 }} Reviews)
        </div>
      </div>
    </div>
    <button
    ...
    <NuxtLink v-if="!loggedIn" to="/login">
      <button class="underline my-5">Login to Write a Review</button>
    </NuxtLink>
